...
| `03-install-npm-globals.sh` | Reloads Homebrew and nvm, enforces Node `24.18.0`, removes conflicting npm prefix settings, installs missing global CLI packages, and enables Corepack. | nvm, exact Node availability, npm registry, package install lifecycle scripts, user `.npmrc`, and Corepack. Registry or install failure aborts. Presence checks do not enforce declared package versions. | Existing package names are skipped; a partial registry run is resumed by rerunning. The duplicated `.npmrc` rewrite has the same recovery limitations as step 02. | One end-to-end fixture covers a clean install against a fake npm implementation. It does not execute a no-op rerun. Version drift, partial installs, and `.npmrc` hazards are not covered. |
...
- 1 npm-global clean-install contract test;
