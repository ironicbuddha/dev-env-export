...
test_profile_expectations_are_complete() {
    ...
    assert_array_contains "chatgpt" "${BOOTSTRAP_REQUIRED_CASKS[@]}"
    assert_array_contains "google-chrome" "${BOOTSTRAP_REQUIRED_CASKS[@]}"
    assert_array_contains "obsidian" "${BOOTSTRAP_REQUIRED_CASKS[@]}"
    assert_array_contains "zoom" "${BOOTSTRAP_REQUIRED_CASKS[@]}"
    assert_array_contains "ChatGPT.app" "${BOOTSTRAP_REQUIRED_APP_BUNDLES[@]}"
    assert_array_contains "Google Chrome.app" "${BOOTSTRAP_REQUIRED_APP_BUNDLES[@]}"
    assert_array_contains "zoom.us.app" "${BOOTSTRAP_REQUIRED_APP_BUNDLES[@]}"
    ...
    assert_array_excludes "affinity" "${BOOTSTRAP_REQUIRED_CASKS[@]}"
    assert_array_excludes "Affinity.app" "${BOOTSTRAP_REQUIRED_APP_BUNDLES[@]}"
...
test_bun_uses_homebrew_core_without_a_third_party_tap() {
    ...
    assert_file_contains "$cli_install" "bootstrap_ensure_bun_fallback || exit 1"
}
